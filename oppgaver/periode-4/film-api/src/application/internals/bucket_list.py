...
class BucketList:
    def __init__(self) -> None:
        ...
        self.__elements: dict[str, MediaElement] = {}

    def add(self, media_element: MediaElement) -> None:
        if media_element.imdb_id not in self.__elements:
            self.__elements[media_element.imdb_id] = media_element.model_dump(
                by_alias=True
            )
    ...
    def get(self) -> list[MediaElement]:
        return list(
            map(lambda element: MediaElement(**element), self.__elements.values())
        )
